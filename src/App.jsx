  ...
  return (
    <div style={{ maxWidth: 1320, margin: '0 auto', padding: '24px 20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em' }}>FC 26 Scout</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 3 }}>18,405 players · 662 clubs · FC 26</p>
        </div>
        <div style={{ display: 'flex', gap: 4, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 4 }}>
          {[
            ...
            <button key={key} onClick={() => setTab(key)} style={{
              padding: '6px 20px', borderRadius: 7, fontSize: 13, fontWeight: 500,
      ...
      {tab === 'players' && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, flex: 1, marginRight: 12 }}>
              {recentlyViewed.length === 0
                ? Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} style={{ background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 68 }}>
                ...
                : <>
                  ...
                  {Array.from({ length: 4 - recentlyViewed.length }).map((_, i) => (
                    <div key={`empty-${i}`} style={{ background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 68 }}>
                      <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>Recently viewed</span>
                    </div>
                  ))}
                </>
